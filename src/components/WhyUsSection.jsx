...
const WhyUsSection = forwardRef(function WhyUsSection({ isVisible }, ref) {
  const features = [
    { icon: imgSvg2, title: "Save Time", desc: "Reduce preparation time by up to 70%" },
    { icon: imgSvg3, title: "Higher Accuracy", desc: "Eliminate data errors with AI validation" },
    { icon: imgSvg4, title: "Complete Transparency", desc: "Track every metric and document" },
    { icon: imgSvg5, title: "15 HandsOn Projects", desc: "Clear path from C to A++" }
  ];
  ...
  return (
    <section id="why-us" className={`relative px-4 md:px-8 lg:px-[60px] xl:px-[105px] py-[60px] md:py-[80px] max-w-[1440px] mx-auto lg:mr-[440px] xl:mr-[520px] transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} ref={ref} data-node-id="79:3133">
      ...
      <p className="text-slate-500 text-[16px] md:text-lg font-normal leading-[24px] md:leading-7 mb-[30px] md:mb-[40px] max-w-[689px]">
        Accreditation made simple, accurate, and stress-free.
      </p>
        ...
        {features.map((item, index) => (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-3 w-full">
              <h3 className={`text-xl font-bold leading-7 text-center w-full transition-colors duration-300 ${
                item.title === "15 HandsOn Projects" 
                  ? "text-black group-hover:text-[#1d9883]" 
                  : "text-slate-900 group-hover:text-[#1d9883]"
              }`}>
                {item.title}
              </h3>
